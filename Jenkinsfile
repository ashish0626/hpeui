...
pipeline {
  environment {
    ...
    DEPLOY_GITREPO_URL = "github.com/${DEPLOY_GITREPO_USER}/hpeui-helmchart.git"
    DEPLOY_GITREPO_BRANCH = "main"
    DEPLOY_GITREPO_TOKEN = credentials('github_ashish')
  }    
  agent {
    kubernetes {
      label "hpeui-${myid}"
...
"""
  ...
  stages {
    ...
    stage('Build Docker Containerize and push to harbor') {
      steps {
        container('kaniko') {
          sh "sed -i 's,harbor.example.com,${env.HARBOR_URL},g' Dockerfile"
          sh "/kaniko/executor --dockerfile Dockerfile --context `pwd` --skip-tls-verify --destination=${env.HARBOR_URL}/library/samples/hpeui:v1.0.${env.BUILD_ID}"
        }
      }
    }
    stage('Image Vulnerability Scan') {
      steps {
        ...
        // writeFile file: 'anchore_images', text: "${env.HARBOR_URL}/library/samples/hpeui:v1.0.${env.BUILD_ID}"
      ...
      steps {
        container('maven') {
          sh """
            ...
            cd deploy
            # update values.yaml
            sed -i -r 's,repository: (.+),repository: ${env.HARBOR_URL}/library/samples/hpeui,' values.yaml
